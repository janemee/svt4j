...
public class ToolDateTime {
    ...
    /**
     * 主要是给jfinal使用，数据库只认java.sql.sql.*
     *
     * @return
     */
    ...
    public static Long getTimeStamp(String datetime) {
        Date date = parse(datetime, "yyyy-MM-dd HH:mm:ss.SSS");
     ...
     * 日期间隔天数
     * @param date 日期
     * @param days 天数  负数表示减去
     * @return 时间
     */
    public static Date getDateIntervalDays(Date date, int days ){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
     ...
     * @return 第一个时间的毫秒数减去第二个时间的毫秒数的差
     */
    public static long compare(Date startDateTime, Date endDateTime) {
...
//        System.out.println(getDateMinuteSpace(sdf.parse("2016-08-20"), sdf.parse("2016-08-19")));

        System.out.println(ToolDateTime.format("20170818","yyyyMMdd","yyyy-MM-dd HH:mm:ss.SSS"));
    }
