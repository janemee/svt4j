...
public class JsGridReportBase {
    ...
    public void outDataToBrowser(TableData tableData) {
        ...
        // 向前台输出数据
        outData.append("{pageInfo: {totalRowNum: ").append(tableData.getTotalRows()).append("},");
        outData.append("data: [");
     ...
     * @return void
     */
    private void stopGrouping(HSSFSheet sheet, HashMap<Integer, Object> word, HashMap<Integer, Integer> counter, int i,
    ...
    public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles,
        ...
        String create_time = formater.format(DateUtil.getNowDate());
        ...
        HashMap<Integer, Object> word = new HashMap<Integer, Object>();
        int index = 0;
        for (TableDataRow dataRow : dataRows) {
            row = sheet.createRow(rownum);
            ...
            for (int i = 0; i < size; i++) {
                ...
                Object value = null;
                if (dataCells.get(i).getCellStyle() == TableData.STYLE_TYPE_FLOAT_2) {
                    value = dataCells.get(i).getDoubleValue();
                    tc.setColumnType(TableData.STYLE_TYPE_FLOAT_2);
                } else {
                    value = dataCells.get(i).getValue();
                }

                if (tc.isGrouped()) {
                    Object w = word.get(index);
                    if (w == null) {
...
//        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
        ...
        switch (tc.getColumnType()) {
            ...
            default:
                if (dc.getValue().equalsIgnoreCase("&nbsp;"))
                    ...
                    cell.setCellValue(dc.getValue());
                if (TableData.STYLE_TYPE_FLOAT_2 == dc.getCellStyle()) {
                    style = styles.get("D2");
                } else {
    ...
    private HashMap<String, HSSFCellStyle> initStyles(HSSFWorkbook wb) {
        ...
        try {
            ...
            InputStream is = this.getClass().getClassLoader().getResourceAsStream("export.xls");
