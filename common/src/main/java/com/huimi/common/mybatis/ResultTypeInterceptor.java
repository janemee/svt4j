...
public class ResultTypeInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        // 通过泛型解析class
        resultType = getResultTypeForPo(args[0]);
        if (resultType == null) {
            resultType = getResultType(parameterObject);
        }
        if (resultType == null) {
...
//
            ...
            Object result = null;
            try {
                result = metaObject.getValue(resultType);
            } catch (Exception e) {
                log.info("查询参数未传入实体类对象class  @Param(\"resultType\")Class resultType 执行默认实体");
                return null;
            }
            return objectToClass(result);
        }
     ...
     * @return
     */
    private Class getResultTypeForPo(Object obj) {
        Class clazz = null;
        try {
            String id = ((MappedStatement) obj).getId();
            Class clazz2 = Class.forName(id.substring(0, id.lastIndexOf(".")));
            clazz = (Class) ((ParameterizedType) clazz2.getClass().getGenericSuperclass()).getActualTypeArguments()[0];
        } catch (Exception e) {
            log.debug("泛型解析失败");
        }
        return clazz;
