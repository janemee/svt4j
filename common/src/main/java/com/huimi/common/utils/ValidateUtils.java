...
//    private static Pattern MOBILE_PATTERN = Pattern.compile("^((13[0-9])|(15[^4,\\D])|(17[0|(18[0-9]),1,3,5,6,7,8])|(18[0-9])|(19[0-9])|(14[5,7])|)\\d{8}$");
    private static Pattern MOBILE_PATTERN = Pattern.compile("^((1[0-9])|)\\d{9}$");
    /**
     * 密码格式1
     */
     ...
     * 0-16位包含数字和英文字母
     */
    private static Pattern ENG_PATTERN1 = Pattern.compile("^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}");
    /**
     * 数字or字符串
     */
     ...
     * 真实姓名
     */
    private static Pattern REAL_NAME = Pattern.compile("^[\\u4e00-\\u9fa5]{2,16}|[\\u4e00-\\u9fa5]+[.][\\u4e00-\\u9fa5]+$");

    private static Pattern REAL_NAME2 = Pattern.compile("^[\\u4E00-\\u9FA5\\uf900-\\ufa2d·s]{2,20}$");
    ...
    public static boolean isPhone(String phone) {
        Matcher matcher = MOBILE_PATTERN.matcher(StringUtils.isNull(phone));
    ...
    public static boolean isPrice3(String price) {
        //toPlainString() 解决科学计数法问题
        price = isNull(price);
        Matcher matcher = PRICE_PATTERN3.matcher(new BigDecimal(price).toPlainString());
        return matcher.matches();
    }
    ...
    private static boolean regular(String str) {
        ...
        Matcher m = ENG_PATTERN1.matcher(str);
        return m.matches();
    }
     ...
     * 9:00,12:00
     */
    public static boolean isTradeTime(String s) {
        String str = "^([0-9]|1[0-9]|2[0-3]):[0-5][0-9],([1-9]|1[0-9]|2[0-3]):[0-5][0-9];([0-9]|1[0-9]|2[0-3]):[0-5][0-9],([0-9]|1[0-9]|2[0-3]):[0-5][0-9]$";
        String str1 = "^([0-9]|1[0-9]|2[0-3]):[0-5][0-9],([1-9]|1[0-9]|2[0-3]):[0-5][0-9]$";
        if (StrUtil.isBlank(s)) {
            return false;
        }
        if (s.trim().matches(str) || s.trim().matches(str1)) {
            return true;
        }
        return false;
    }
     ...
     * @return 是否满足调价
     */
    public static boolean isInteger(Integer s) {
        return s != null && !"".equals(s.toString().trim()) && s.toString().trim().matches("^+?(([1-9]\\d?))$");
    }
    ...
    public static boolean isUserName(String userName) {
        Pattern p = Pattern.compile("^(?!_)(?!.*?_$)[\\u4e00-\\u9fa5]+$");
    ...
    public static boolean isRealName(String userName) {
        if (userName.contains("·")) {
            return REAL_NAME2.matcher(userName).matches();
        }
        return REAL_NAME.matcher(userName).matches();
    }
}
