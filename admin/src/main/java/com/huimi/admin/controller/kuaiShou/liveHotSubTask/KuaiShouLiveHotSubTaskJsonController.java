...
public class KuaiShouLiveHotSubTaskJsonController extends BaseController {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(KuaiShouLiveHotSubTaskJsonController.class);
    ...
    public ResultEntity addSuperHeat(String deviceStyle, String[] equipments, String[] equipmentGroups, Task task, String taskStartTime) throws BussinessException {
        try {
            //检查直播链接是否正确
            if (StringUtil.isNotBlank(task.getLiveInContent()) && StringUtil.isBlank(TaskUtil.saveLiveInContentKwaiFu(task.getLiveInContent()))) {
            ...
            if (StringUtils.isNotBlank(task.getLiveInContent())) {
                task.setAnalysisContent(TaskUtil.saveLiveInContentKwaiFu(task.getLiveInContent()));
            }
            long s2 = System.currentTimeMillis();
            String millis = System.currentTimeMillis() + "";
            task.setPlatform(EnumConstants.PLAT_FROM_TYPE.KUAISHOU.value);
    ...
    public ResultEntity addImmediatelyTask(TaskAdminPramsModel taskAdminPramsModel) {
        try {
            ...
            taskAdminPramsModel.setPlatform(EnumConstants.PLAT_FROM_TYPE.KUAISHOU.value);
